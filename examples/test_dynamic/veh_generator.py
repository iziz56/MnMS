...
def veh_generation(rh_service, nb_veh):
    ...
    name = 'veh_locations_' + str(rh_service.id) + '.csv'
    df.to_csv(name, mode='w', sep=';', index=False)

def veh_read_uber(rh_service):
    df = pd.read_csv('veh_locations_UBER.csv', sep=';')
...
def veh_read_lyft(rh_service):
    df = pd.read_csv('veh_locations_LYFT.csv', sep=';')
